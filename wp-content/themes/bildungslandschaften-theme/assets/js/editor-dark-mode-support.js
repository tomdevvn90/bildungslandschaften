/* global bildungslandschaftenIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'bildungslandschaften-supports-dark-theme' ) ) {
	bildungslandschaftenDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function bildungslandschaftenDarkModeEditorInit( attempt ) {
	...
	if ( bildungslandschaftenIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						bildungslandschaftenDarkModeEditorInit( attempt + 1 );
					},
